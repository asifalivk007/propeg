...
const MOTIF_ANCHOR_SENSE = {
    7:  { x: 235, y: 173, a: 82 },
    8:  { x: 226, y: 178, a: 82 },
    9:  { x: 217, y: 182, a: 82 },
    10: { x: 208, y: 188, a: 97 },
    11: { x: 199, y: 189, a: 102 },
    12: { x: 187, y: 192, a: 107 },
    13: { x: 179, y: 192, a: 107 },
    14: { x: 193, y: 192, a: 107 },
    15: { x: 181, y: 193, a: 114 },
    16: { x: 172, y: 194, a: 120 },
    17: { x: 175, y: 195, a: 123 },
};
...
export default ({ spacer = '', pbs = '', rtTemplate = '', scaffoldMod = '', linker = '', motifX, motifY, motifAngle }) => {
    ...
    // Long-RT layout (arms grow + font shrinks, both every 12 nt from 30) — shared
    // policy in rtArmAndFont so all components stay in sync.
    ...
    // The sense linker is anchored at its LEFT edge and grows rightward toward the PBS, so
    // its PBS-facing (right) edge is set by the glyph width. Shrinking the font narrows it
    // and the right edge retreats from the PBS, so nudge the linker rightward to stay snug.
    // The narrower the font, the larger the nudge needed: ~25 px at 10 px, ~36 px at 8 px.
    const linkerXShift = linkerFontPx === 8 ? 36 : linkerFontPx === 10 ? 25 : 0;

    // No transform scaling (arms stay attached to scaffold/PBS); just extend the viewBox
    // top enough to fit the loop apex when RT is long. Smaller font + growing arms keep
    // the extension modest.
    ...
    // S1 (4bp): pos 1-4 paired with 19-22
    // Circle (14nt hairpin): pos 5-18
    // L3 curved path: pos 23-29
    // S2 arc (7bp): pos 30-36 paired with 17-11
    // pos 37: unpaired 3' end
    ...
    // Anchor: we generate motif in standard local coords then rotate/translate.
    // Local coords: S1 base at (0,0), S1 goes UP (negative Y).
    const tPos = {};
    const TS = 8;   // spacing between nts
    const TG = 14;  // gap between paired strands
    const TR = 22;  // tevo circle radius

    // S1 (4bp) going straight up
    for (let i = 0; i < 4; i++) {
        const d = (i + 1) * TS;
        tPos[1 + i] = { x: -TG / 2, y: -d };
    ...
    // Circle center: above S1 top
    const circCY = s1TopY - TR * 1.4;
    const circCX = 0; // centered

    const gapH = 0.28;
    // In local coords, up is -Y. The gap is at the bottom of the circle (facing the stem).
    // angle 90 degrees (Math.PI/2) is DOWN in normal SVG, UP is -Math.PI/2
    ...
    // S2 arc (7bp): 30-36 outside circle, radially aligned with 17-11
    const s2R = TR + 11;
    for (let i = 0; i < 7; i++) {
        const partner = 17 - i;
    ...
    // L3 curved path (pos 23-29): bezier from 22 down-around to 30
    const l3S = tPos[22];
    const l3E = tPos[30];
    ...
    // pos 37: unpaired A extending from 36
    ...
    // Transform local coords to final viewBox coords
    // The motif attaches at the end of the linker.

    // Anchor + tilt come from the per-PBS calibration table (clamped to the tuned 7–17
    // range). The harness can still override any of the three live via the props.
    const mp = MOTIF_ANCHOR_SENSE[Math.max(7, Math.min(17, pbsLength))];

    // Parameter to easily rotate the entire motif attached to the scaffold
    ...
    // Orient the motif's local frame (local UP / -Y points away from the strand).
    // This sets only the motif's rotation; the anchor position is computed above.
    const rotA = rotAng - Math.PI / 2;
    const cosA = Math.cos(rotA);
    const sinA = Math.sin(rotA);
    const tevoAngleDeg = {}; // store the resulting angle needed for each letter
    // Global scaffold rotation + motif structural rotation + 180 flip to match scaffold style
    const globalAngleDeg = angleDeg + 90;

    for (let i = 1; i <= 37; i++) {
        if (tPos[i]) {
            // We multiply tPos[i].x by -1 to mirror the entire structure over the local Y (vertical) axis
            const mirroredX = -tPos[i].x;
            ...
            // To align the letters properly with their position round the loops:
    ...
    >
        ...
        <g>
            ...
            <g>
                ...
                <text transform="rotate(-30 434.94 -420.564)">
                    <tspan x={(13 - pbsLength - linkerLength) * 10 - 12 + linkerXShift} y="0" className="st12 st7" style={linkerFontStyle}>
                        {reverseSequence(linker)}
                    </tspan>
        ...
        {/* === tevopreQ1 pseudoknot secondary structure === */}
        {motif && <g>
            {/* S1 base pair lines */}
            ...
            {motif.split('').map((nt, i) => {
                ...
                const p = tevoPos[idx];
                if (!p) return null;
                // Add transform rotate like the scaffold
            ...
            {tevoPos[37] && <text x={tevoPos[37].x - 16} y={tevoPos[37].y - 3}
                className="st5 st6"
